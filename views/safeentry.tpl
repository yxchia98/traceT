...
<head>
  ...
  <link type=""text/css" href="/static/css/safeE.css" rel="stylesheet">
...
<div class="jumbotron text-center">
  <h4>This is for demonstration purpose only.</h4>
  <h1>SafeEntry</h1>
...
<div class="container" style="font-size: 150%">
  ...
  <div class="card" style="border-background-left-radius: 25px">
	...
	<img class="card-background" src="https://theme.zdassets.com/theme_assets/9722682/31cf771430adf7e5fc91e10d20819cb263c68e1d.png">
	  <p class="current date" id="currentDate">{{date}}</p>
	  <p class="current time" id="currentTime">{{time}}</p>
	  <p class="current location">{{location}}</p>
	</div>
  </div>
    <form method="post" action="/safeout">
      <input type="hidden" id="id" name="id" value="{{id}}"><br>
      <input type="hidden" id="datetime" name="datetime" value="{{datetime}}"><br>
      <input type="hidden" id="location" name="location" value="{{location}}"><br>
      <div class="checkinout">
      <button type="submit" class="b2">CHECK OUT</button>
      </div>
    </form>
